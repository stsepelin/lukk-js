...
describe('revokeDroppedSession', () => {
  it('logs the dropped session out with the access token its rotation just minted', async () => {
    ...
    revokeDroppedSession(event(waitUntil), { access: 'A2', refresh: 'rA2' }, 'https://lukk.test/auth', '198.51.100.7')

    expect(fetchSpy).toHaveBeenCalledWith('https://lukk.test/auth/logout', {
      method: 'POST',
      headers: { 'Accept': 'application/json', 'Content-Type': 'application/json', 'Authorization': 'Bearer A2', 'X-Forwarded-For': '198.51.100.7' },
  ...
  it('names no visitor when it has none, and runs without waitUntil', () => {
    ...
    revokeDroppedSession(event(), { access: 'A2' }, 'https://lukk.test/auth')
    ...
    revokeDroppedSession(event(), { refresh: 'rA' }, 'https://lukk.test/auth')
  ...
  it('sends nothing without a token to revoke with, or to an unusable base', () => {
    ...
    revokeDroppedSession(event(), {}, 'https://lukk.test/auth')
    revokeDroppedSession(event(), { access: 'A2' }, 'undefined/auth')
