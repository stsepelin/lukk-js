...
Also fixed: after `clearNuxtState()`, an existing `useLukkAuth()` read `loggedIn` and `pendingTwoFactor` as `true`. And `useLukkAuth()` now has a declared return type, `LukkAuth` — the published declarations typed every field `any`, so `auth.ready.value = true` type-checked. **This can fail a typecheck that passed before:** code that leaned on `any` — `user.value.name` without a null check, an assignment to `loggedIn` — now reports an error. The other composables (`useLukkConfirmation`, `useLukkAbilities`, `useLukkTwoFactor`, `useLukkPasskeys` and more) still publish `any` fields.
