...
A tab that signs in or logs out now tells the app's other open tabs through a `BroadcastChannel` (where the browser has one). They drop what they had in flight and re-check — a BFF tab reloads its user, a direct tab renews from the shared cookie first — instead of showing an account the browser no longer holds while their requests act as another. In direct mode, a logout in another tab still clears the shared refresh cookie, including one a sign-in in this tab has just set — that new session then stays alive on lukk, unreachable, until it expires. Several tabs following a sign-in renew from the same cookie at once, which relies on lukk's grace window: keep `grace_seconds` above `0`. A sign-in that issues a session, and every `logout()` before it sends its request, starts a new session generation; a refresh, restore or `fetchUser()` from an older one discards its result instead of writing it. A sign-in that answers after a `logout()` ends the session it was issued rather than signing in. One that started no session (rejected credentials, a two-factor challenge, a registration awaiting verification) changes nothing.
